...
# 축구 게임 홈페이지에서 능력치 가져옮
    def set_stat_auto(self):
        ...
        # tr 에 있는 td 를 찾아 거기에 포함된 text 들을 data list 에 저장시켰습니다. 이 값들이 스탯 창에 있는 이용하고자 하는 수치들입니다.
        data = []
        ...
        # 해당 값들이 data 리스트의 어디에 있는지는 직접 테스트해보며 확인하였습니다.
        if self.p_position == 'GK':
...
def set_start_random(team1):
    ...
    # 선수들은 11명이므로, 11명을 while 문을 이용해서 포지션과 스탯 랜덤하게 설정.
    # 스탯 설정은 Player 클래스의 set_stat_random 을 이용
    while len(players_list) != 11:
        ...
        # 포지션도 랜덤으로 설정하기 위해 not in 구문을 사용함
...
# 선수들은 11명이므로, 11명 범위 내에서 Player 객체를 11개 만들고
# 스탯은 Player 클래스의 set_stat_auto 을 이용
def set_start_auto(team1):
    p_list = []
    players = team_dict[team1]
    count = 0

    # 11명 선수의 값들을 할당. random 이 아니고 해당 홈페이지에서 값들을 가져옴
    # 따라서 position 에는 별다른 값을 할당하지 않음. 홈페이지에서 할당시켜줄 것이기 때문
    for i in range(0, 11):
    ...
    # 웹 스크랩 시 텀이 없으면 ip를 차단당하거나 오류가 발생할 확률이 있음. time.sleep(1) 을 해줌으로써 해결
    # 이를 시각적으로 표현하고자 print 문을 활용해서 완료 정도를 %로 나타내었음
    for p in p_list:
